...
public class Block {
	...
	public static final Block button = (new BlockButtonVanilla(77, stone.blockIndexInTexture)).setHardness(0.5F).setStepSound(soundStoneFootstep);
		...
		if(!worldObj.multiplayerWorld) {
			...
			for(int i8 = 0; i8 < i7; ++i8) {
				if(worldObj.rand.nextFloat() <= chance) {
					...
					if(i9 > 0) {
						EntityItem entityItem11 = new EntityItem(worldObj, (double)x + (double)(worldObj.rand.nextFloat() * 0.7F) + (double)0.15F, (double)y + (double)(worldObj.rand.nextFloat() * 0.7F) + (double)0.15F, (double)z + (double)(worldObj.rand.nextFloat() * 0.7F) + (double)0.15F, new ItemStack(i9));
	...
	public boolean altG(World world1, int i2, int i3, int i4) {
		boolean z5 = i2 == grass.blockID || i2 == dirt.blockID || i2 == tilledField.blockID;
		return (world1.getBlockLightValue(i2, i3, i4) >= 8 || world1.canBlockSeeTheSky(i2, i3, i4)) && z5;
	}
